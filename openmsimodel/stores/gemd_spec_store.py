...
class GEMDSpecStore:
    ...
    def unique_version_of(
        self, specobj, debug=False, recursive_check=True, recursive_register=True
    ):
        """
        ...
        if recursive_register:
            recursive_foreach(
                specobj, self.__register_new_unique_specs, apply_first=True
            )
        else:
    ...
    def remove_unneeded_spec(self, item):
        ...
        if name not in dict_to_use.keys():
            raise RuntimeError(
                f"ERROR: no {type(item).__name__} with name {name} found in spec store!"
            )
        elif uid not in dict_to_use[name].keys():
            raise RuntimeError(
                f"ERROR: no {type(item).__name__} {name} with UID {uid} found in spec store!"
            )
    ...
    def __get_stored_version_of_spec(self, specobj, debug=False):
        new_spec_name, new_spec_as_dict_no_uid = self.__get_name_and_dict_for_spec(
            specobj
        )
    ...
    def __register_spec(self, item, from_file=False):
        ...
        if self.__debug:
            print(
                f"Creating a new {type(item).__name__} with name {new_spec_name} ({new_uid})"
            )
        dict_of_type[new_spec_name][new_uid] = GEMDSpec(
