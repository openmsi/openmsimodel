...
class BaseNode(ABC):
    ...
    def _update_attributes(
        ...
        update_attrs(self._ATTRS, self._spec, self._run, AttrType, attributes, replace_all, which)
    ...
    def get_filelinks_dict(self) -> FileLinksDict:
        ...
        filelinks_dict['spec'] = tuple(self._link_str(link) for link in self._spec.file_links)
        filelinks_dict['run'] = tuple(self._link_str(link) for link in self._run.file_links)
    ...
    def _remove_filelinks(
        ...
        spec_or_run.file_links = [
            link for link in spec_or_run.file_links
